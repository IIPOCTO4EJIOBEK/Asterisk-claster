...
TASKS = [
 ("1", "Подготовка: сеть, серверы, договорённости с операторами", (1,2), 8,
  ...
  "VPN между площадками поднят, RTT измерен и зафиксирован, пятый сервер под Москву "
  "получен, у каждой площадки свой SIP-пользователь ВАТС",
  [("1.1","Утвердить единый план нумерации, разослать сотрудникам новые номера",(1,1),1,
   ...
   ("1.4","Получить IP-адреса Славянска-на-Кубани и локального PBX",(1,1),1,
    "Адреса получены"),
   ("1.5","Подготовить сервер под московский узел",(1,2),1,
    "Сервер получен и доступен по сети"),
...
def add_params_sheet(wb, kind):
    ...
    rows = [
        ...
        ("Плановая себестоимость внешних затрат, ₽ без НДС", 0,
         ...
         "OSS PBX End Point Manager. Сервер под московский узел проходит закупкой, "
        ...
        ("Серверов нужно докупить", 1,
         "ДОПУЩЕНИЕ: «у нас 5 серверов» прочитано как пять узлов кластера. Четыре "
         "площадки уже имеют свои машины, новый сервер нужен только под Москву. "
         "Ставрополь (74 абонента) остаётся на ростовском сервере, как сейчас."),
        ("Резервный мастер отдельной машиной", "нет",
         "Пятый сервер занят московским узлом. Отдельная машина под тёплый резерв не "
         "требуется: каждый узел — клон мастера, FreePBX на нём уже установлен и только "
         "отключён, поэтому роль мастера передаётся на любой рабочий узел. Процедура "
         "отрабатывается на этапе 7 учебно."),
    ]
    r = 4
    for a, b, c in rows:
